...
UPDATE public.notes
  SET link_excerpt_tsv = to_tsvector(get_current_ts_config(), coalesce(link_excerpt, ''));
...
RETURNS TRIGGER AS $$
BEGIN
  new.link_excerpt_tsv := to_tsvector(get_current_ts_config(), coalesce(new.link_excerpt, ''));
  RETURN new;
END;
$$ LANGUAGE plpgsql;
